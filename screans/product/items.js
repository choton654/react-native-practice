  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      <View style={{ flex: 1 }}>
        {state.products.map(
          (prod) =>
            prod.category._id.toString() === subcatId.toString() && (
              <View style={{ flex: 1 }} key={prod._id}>
                <Card
                  style={{ marginVertical: 20, margin: "auto", width: "80%" }}
                  onPress={() =>
                    navigation.navigate("SingleItem", { itemId: prod._id })
                  }>
                  <Card.Content>
                    <Title>{prod.name}</Title>
                  </Card.Content>
                  <Card.Cover
                    source={{ uri: `${BASE_URL}${prod.photo[0].img}` }}
                  />
                </Card>
              </View>
            )
